...
//якщо ot нема в localStorage  записуємо пусті масиви
...
//в onFirstRender  записується масив жанрів в localStorage
//записуюємо користувачу в localStorage список жанрів
function saveGenres(genres) {
  //*******ЖАНРИ******
  //записуємо  масив жанрів в localStorage якщо його нема або змінилась мова
  save(GENRES_KEY, genres);
}
//вигружаємо розпарсені жанри
function loadGenres() {
...
//весь масив сторінки трендових фільмів можна записати в current localStorage
function saveTrendMovies(page) {
  remove(CURRENT_KEY);
  apiMovie.fetchAllMovie(page) //day
    .then(data => {
...
function loadCurrentPage() {
  const movies = load(CURRENT_KEY);
  //console.log('from loadCurrentPage', movies);
  return movies;
}
