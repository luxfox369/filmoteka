...
//export
  function isWatched(movie, event) {
...
export function addToWatched(movie, event) {
  ...
  const watchedMovies = storage.load(WATCHED_KEY);
  if (
    watchedMovies.some(film => film.id === movie.id) &&
    event.target.textContent === addWatched
  ) {
    //
    watchedMovies.push(movie);
    storage.save(WATCHED_KEY, watchedMovies);
    event.target.textContent = removeWatched;
...
function addToQueue(movie, btn) {
  ...
  const queueMovies = storage.load(QUEUE_KEY);
  if (!queueMovies.includes(movie) && btn.textContent === addQueue) {
   queueMovies.push(movie);
    storage.save(QUEUE_KEY, queueMovies);
...
function removeFromQueue(movie, btn) {
  //btn це ref на кнопку в модалці  QUEUE
  const queueMovies = storage.load(QUEUE_KEY);
  if (queueMovies.includes(movie) && btn.textContent === removeQueue) {
   queueMovies = queueMovies.filter(({ id }) => id !== movie.id);
    storage.save(QUEUE_KEY, queueMovies);
